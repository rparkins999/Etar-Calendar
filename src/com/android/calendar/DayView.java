    ...
    View.OnLongClickListener
{
    ...
    //private static int MIN_HOURS_WIDTH = 96; (never read)
    ...
    //private static int CALENDAR_COLOR_SQUARE_SIZE = 10; (never read)
     ...
     */
    private static final int ALPHA_SELECTION_OVER_EVENT = 127;
    /**
    ...
    // Another one, private to compareToVisibleTimeRange and find{Right,Left}Event
    ...
    // These are set by setSelectionFromPosition()
    ...
    private final Runnable mTZUpdater = new Runnable() {
        ...
        public void run() {
            ...
            mSelectionTime.switchTimezone(tz);
            invalidate();
        }
    };
     ...
     * This is a class variable to save reallocating it inside onDraw()
     */
    ...
    public DayView(Context context, CalendarController controller,
        ...
        //MIN_HOURS_WIDTH = (int) mResources.getDimension(R.dimen.min_hours_width); (never read)
        ...
        if (mScale == 0) {
            ...
            if (mScale != 1) {
                ...
                //CALENDAR_COLOR_SQUARE_SIZE *= mScale; (never read)
        ...
        int multiDayHeaderHeight
            = (int) mResources.getDimension(R.dimen.day_header_height);
        DAY_HEADER_HEIGHT = mNumDays == 1 ? ONE_DAY_HEADER_HEIGHT : multiDayHeaderHeight;
    ...
    private void setSelectionTime(int julianDay, int hour) {
        //Llog.d("","" + julianDay + ", " + hour);
        mSelectionJulianDay = julianDay;
        mSelectionTime.setJulianDay(julianDay);
        mSelectionTime.hour = hour;
        mSelectionTime.minute = 0;
        ...
        mSelectionTime.normalize(true);
        //Llog.d(mSelectionTime.format("mSelectionTime set to %b%d %H:%M"));

    }

    private void setSelectionTime(long millis) {
        // set mSelectionTime to start of first hour >= current time
        //Llog.d("","" + millis);
        mSelectionTime.set(millis);
        if ((mSelectionTime.minute > 0) || (mSelectionTime.second > 0)) {
            mSelectionTime.hour++;
            mSelectionTime.minute = 0;
            mSelectionTime.second = 0;
            mSelectionTime.normalize(true);
        }
        mSelectionJulianDay = Time.getJulianDay(
            mSelectionTime.toMillis(true), mSelectionTime.gmtoff);
        //Llog.d(mSelectionTime.format("mSelectionTime set to %b%d %H:%M"));
    }
    ...
    private void init(Context context) {
        ...
        int gridLineColor =
            mResources.getColor(R.color.calendar_grid_line_highlight_color);
        ...
        String[] dayStrs2Letter = new String[14];
        ...
        for (int i = Calendar.SUNDAY; i <= Calendar.SATURDAY; i++) {
            ...
            // e.g. Tu for Tuesday
            dayStrs2Letter[index] = DateUtils.getDayOfWeekString(i, DateUtils.LENGTH_SHORT);

            // If we don't have 2-letter day strings, fall back to 1-letter.
            if (dayStrs2Letter[index].equals(mDayStrs[index])) {
        ...
        // We only initialise this when the first DayView is created:
        // the java specification says that it will start off null.
        if (mSelectionTime == null) {
            mSelectionTime = new Time(Time.getCurrentTimezone());
            setSelectionTime(millis);
        }

        // set mBaseDate to start of current day
        ...
        mSkippedAlldayEvents = new int[mNumDays];
    ...
    public void updateTitle() {
        ...
        mController.sendAction(this, ControllerAction.UPDATE_TITLE, mBaseDate, mTempTime, null, -1, ViewType.CURRENT,
    ...
    private void adjustCellHeight() {
        ...
        int minCellHeight = (mViewHeight - mFirstCell) / 24;
        mCellHeight = mPreferredCellHeight;
    ...
    private void remeasure() {
        ...
        MAX_UNEXPANDED_ALLDAY_HEIGHT =
            Math.min(MAX_UNEXPANDED_ALLDAY_HEIGHT, mViewHeight / 6);
        ...
        for (int day = 0; day < mNumDays; day++) {
            mEarliestStartHour[day] = Integer.MAX_VALUE;
        }
        ...
        if (maxAllDayEvents > 0) {
            int maxAllAllDayHeight = mViewHeight - DAY_HEADER_HEIGHT - MIN_HOURS_HEIGHT;
        ...
        mGridAreaHeight = mViewHeight - mFirstCell;
    ...
    private void switchViews() {
        ...
        if (mNumDays > 1) {
            ...
            if (selectedEvent == null) {
                ...
                mController.sendEventActionWithExtra(this, ControllerAction.CREATE_EVENT, -1,
    ...
    // This does the work for nextRight().
    // In the case where there is a selected event, but it cannot find an event
    // to the right of it in the selected day, it gets called a second time.
    // Returns true if it found an event
    @SuppressLint("DefaultLocale")
    private boolean findRightEvent(boolean allowSelected) {
        if (DEBUG_DPAD) {
            Llog.d("***********************************************");
        }
        ...
        if (mClickedAllday) {
            if (DEBUG_DPAD) {
                if (mSelectedEvent == null) {
                    Llog.d(mTempTime.format(
                        "No previously selected event, looking for first all day event on %b%d"));
                } else {
                    selectedName = ((mSelectedEvent.title == null)
                        ? "Untitled event" : mSelectedEvent.title).toString();
                    Llog.d(mTempTime.format("Looking for first all day event on %b%d after "
                        + selectedName));
                }
            }
            for (int i = 0; i < numEvents; i++) {
                ...
                if (mSelectedEvent != null) {
                    if (e.equals(mSelectedEvent)) {
                            ...
                            && (mSelectedEvent.endDay == mClickedDay))
                        {
                            ...
                            {
                                if (DEBUG_DPAD) {
                                    Llog.d("Selected event " + selectedName +
                                        " ends after " + eventName + ", not wanted");
                                }
                                continue;
                            }
                                               ...
                                               && (e.getColumn() <= mSelectedEvent.getColumn())))
                        {
                            if (DEBUG_DPAD) {
                                Llog.d("Selected event " + selectedName +
                                    " is after event " + eventName + ", not wanted");
                            }
                            continue;
                        }
                    }
                ...
                if (mClickedEvent != null) {
                    ...
                    if (   ((mClickedEvent.startDay < mClickedDay) == (e.startDay < mClickedDay))
                        && (   (eStartMillis > mClickedEvent.startMillis)
                            || (   (eStartMillis == mClickedEvent.startMillis)
                                && (eEndMillis >= mClickedEvent.endMillis))))
                    {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName
                                + " is after best so far " + clickedName + ", not wanted");
                        }
                        continue;
                    }
                    if (   (mClickedEvent.startDay == mClickedDay)
                        && (e.startDay != mClickedDay))
                    {
                        if (DEBUG_DPAD) {
                            mCompareTime.setJulianDay(e.startDay);
                            Llog.d("best so far " + clickedName
                                + mTempTime.format(" starts on %b%d and ")
                                + eventName + mCompareTime.format(" starts on %b%d, not wanted"));
                        }
                        continue;
                    }
                }
            ...
            if (DEBUG_DPAD) {
                if (mSelectedEvent == null) {
                    Llog.d(mTempTime.format(
                        ...
                        + "looking for rightmost event during %H:00 on %b%d"));
                } else {
                    selectedName = ((mSelectedEvent.title == null)
                        ? "Untitled event" : mSelectedEvent.title).toString();
                    Llog.d(mTempTime.format(
                        "Looking for event during %H:00 on %b%d to right of "
                        + selectedName));
                }
            }
            mTempTime.hour += 1;
            ...
            for (int i = 0; i < numEvents; i++) {
                ...
                if (eEndMillis <= startMillis) {
                    if (DEBUG_DPAD) {
                        mCompareTime.set(startMillis);
                        Llog.d(eventName
                            + mCompareTime.format(" is before %b%d %H%M, not wanted"));
                    }
                    continue;
                }
                if (eStartMillis >= endMillis) {
                    if (DEBUG_DPAD) {
                        mCompareTime.set(endMillis);
                        Llog.d(eventName
                            + mCompareTime.format(" is after %b%d %H%M, not wanted"));
                    }
                    continue;
                }
                int column = e.getColumn();
                if (mSelectedEvent != null) {
                    if (eEndMillis <= mSelectedEvent.startMillis) {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName + " is before " + selectedName + ", not wanted");
                        }
                        continue;
                    }
                    if ((column <= mSelectedEvent.getColumn())
                        ...
                        if (DEBUG_DPAD) {
                            Llog.d(eventName + " is in column " + column +
                                " not after " + selectedName + " in column " +
                    ...
                    && (column < mClickedEvent.getColumn()))
                {
                    if (DEBUG_DPAD) {
                        ...
                        Llog.d(eventName + " now best so far");
                    }
    ...
    // This gets called on a DPAD_RIGHT key down, which may be a repeat.
    // It selects the nearest event to the right of the currently selected event
    // if there is one, or the currently selected hour if no event is selected.
    // If there is no event within one hour to the right, it selects the next hour
    // to the right.
    ...
    private void nextRight() {
        ...
        mTempTime.setJulianDay(mClickedDay);
        if (DEBUG_DPAD) {
            if (findRightEvent(true)) {
                String eventName = ((mClickedEvent.title == null)
                    ? "Untitled event" : mClickedEvent.title).toString();
                Llog.d("Returning with clicked event " + eventName);
            } else {
                if (mClickedAllday) {
                    Llog.d(mTempTime.format("No event found, just selecting %b%d"));
                } else {
                    mTempTime.hour = mClickedHour;
    ...
    // This does the work for nextLeft().
    // In the case where there is a selected event, but it cannot find an event
    // to the left of it in the selected day/hour, it gets called a second time.
    // Returns true if it found an event
    @SuppressLint("DefaultLocale")
    private boolean findLeftEvent(boolean allowSelected) {
        if (DEBUG_DPAD) {
            Llog.d("***********************************************");
        }
        String selectedName = null;
        ...
        if (mClickedAllday) {
            if (DEBUG_DPAD) {
                if (mSelectedEvent == null) {
                    Llog.d(mTempTime.format(
                        "No previously selected event, looking for last all day event on %b%d"));
                } else {
                    selectedName = ((mSelectedEvent.title == null)
                        ? "Untitled event" : mSelectedEvent.title).toString();
                    Llog.d(mTempTime.format("Looking for last all day event on %b%d before "
                        + selectedName));
                }
            }
            for (int i = 0; i < numEvents; i++) {
                ...
                if (mSelectedEvent != null) {
                    if (e.equals(mSelectedEvent)) {
                        ...
                        {
                            if (DEBUG_DPAD) {
                                Llog.d(eventName + " is after selected event "
                                    + selectedName + ", not wanted");
                            }
                            continue;
                        }
                ...
                if (mClickedEvent != null) {
                    String clickedName = ((mClickedEvent.title == null)
                        ? "Untitled event"
                        : mClickedEvent.title).toString();
                    if (   ((mClickedEvent.endDay > mClickedDay) == (e.endDay > mClickedDay))
                        && (   (eEndMillis < mClickedEvent.endMillis)
                            || (   (eEndMillis == mClickedEvent.endMillis)
                                && (eStartMillis <= mClickedEvent.startMillis))))
                    {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName
                                + " is before best so far " + clickedName + ", not wanted");
                        }
                        continue;
                    }
                    if (   (mClickedEvent.startDay == mClickedDay)
            ...
            if  (mSelectedEvent == null) {
                if (DEBUG_DPAD) {
                    Llog.d(mTempTime.format(
                        "No previously selected event, looking for"
                        + " leftmost event during %H:00 on %b%d"));
                }
            } else {
                if (DEBUG_DPAD) {
                    Llog.d(mTempTime.format(
    ...
    // if there is one, or the currently selected hour if no event is selected.
    // If there is no event within one hour to the left, it selects the next hour
    // to the left.
    // mClickedAllday, mClickedDay, mClickedHour, and mClickedEvent are set to reflect
    // what was found.
    ...
    // This does the work for nextUp().
    // In the case where there is a selected event, but it cannot find an event
    // above it in the selected day, it gets called a second time with allowSelected true.
    // Returns true if it found an event
    private boolean findUpEvent(boolean allowSelected) {
        if (DEBUG_DPAD) {
            Llog.d("***********************************************");
        }
        String selectedName = null;
        ...
        if (mClickedAllday) {
            if (DEBUG_DPAD) {
                if (mSelectedEvent == null) {
                    Llog.d(mTempTime.format(
                        "No previously selected event, looking for lowest all day event on %b%d"));
                } else {
                    selectedName = ((mSelectedEvent.title == null)
                        ? "Untitled event" : mSelectedEvent.title).toString();
                    Llog.d(mTempTime.format("Looking for lowest all day event on %b%d above "
                        + selectedName));
                }
            }
            int minColumn = -1;
            int maxColumn =
                ((mSelectedEvent != null) && mSelectedEvent.drawAsAllday())
            ...
            for (int i = 0; i < numEvents; i++) {
                ...
                if (e.drawAsAllday()) {
                    ...
                    if (e.startDay > mClickedDay) {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName
                                + mTempTime.format(" is after %b%d, not wanted"));
                        }
                        continue;
                    }
                    if (e.endDay < mClickedDay) {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName
                                + mTempTime.format(" is before %b%d, not wanted"));
                        }
                        continue;
                    }
                    int eColumn = e.getColumn();
                    if (eColumn > maxColumn) {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName + " is below selected event, not wanted");
                        }
                        continue;
                    }
                    if (eColumn < minColumn) {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName + " is above best so far, not wanted");
                        }
                        continue;
                    }
                    if (DEBUG_DPAD) {
                        Llog.d(eventName + " in column " + eColumn + " is now best so far");
                    }
                    minColumn = eColumn;
                    mClickedEvent = e;
                } else {
                    if (DEBUG_DPAD) {
                        Llog.d(eventName + " not all day event, not wanted");
                    }
                    continue;
                }
            }
            ...
            long startMillis = mTempTime.toMillis(false);
            mCompareTime.set(mTempTime);
            mCompareTime.hour += 1;
            if (DEBUG_DPAD) {
                if (mSelectedEvent == null) {
                    Llog.d(mTempTime.format("Looking for latest event between %H:00") +
                        mCompareTime.format(" and %H:00 on %b%d"));
                } else {
                    selectedName = ((mSelectedEvent.title == null)
                        ? "Untitled event" : mSelectedEvent.title).toString();
                    Llog.d(mTempTime.format("Looking for latest event between %H:00") +
                        mCompareTime.format(" and %H:00 on %b%d before " + selectedName));
                }
            }
            ...
            for (int i = 0; i < numEvents; i++) {
                ...
                if (eStartMillis >= endMillis) {
                    if (DEBUG_DPAD) {
                        Llog.d(eventName
                            + mCompareTime.format(" starts after %b%d %H:00, not wanted"));
                    }
                    continue;
                }
                if (eEndMillis <= startMillis) {
                    if (DEBUG_DPAD) {
                        Llog.d(eventName
                            + mTempTime.format(" ends before %b%d %H:00, not wanted"));
                    }
                    continue;
                }
                if (mSelectedEvent != null) {
                    ...
                    if (   (eStartMillis >= startMillis)
                        && (mSelectedEvent.startMillis >= startMillis)
                        && (eEndMillis >= endMillis)
                        && (mSelectedEvent.endMillis >= endMillis)
                        && (!e.equals(mSelectedEvent))
                        && !allowSelected)
                    {
                        if (   (eStartMillis > mSelectedEvent.startMillis)
                            || (   (eStartMillis == mSelectedEvent.startMillis)
                        ...
                        if (DEBUG_DPAD) {
                            Llog.d(eventName +
                                ", not wanted because it is after " + selectedName);
                        }
                        continue;
                    }
                }
                        ...
                        if (eStartMillis < mClickedEvent.startMillis) {
                            ...
                            continue;
                               ...
                               || (   (eEndMillis == mClickedEvent.endMillis)
                                   && (   (eStartMillis < mClickedEvent.startMillis)
                                       || (   (eStartMillis == mClickedEvent.startMillis)
                                           && (e.getColumn() < mClickedEvent.getColumn())))))
                    {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName +
                                ", not wanted because it is before " + clickedName);
                        }
                        continue;
                    }
                }
    ...
    // This gets called on a DPAD_UP key down, which may be a repeat
    // It selects the nearest event above the currently selected event
    // if there is one, or the currently selected hour if no event is selected.
    // If there is no event within one hour upwards, it selects the next hour up.
    // If we move up from hour 00, we go into the all day event area.
    // mClickedAllday, mClickedDay, mClickedHour, and mClickedEvent are set to reflect
    // what was found.
    private void nextUp() {
        mClickedAllday = mSelectionAllday;
        ...
        if (mSelectedEvent != null) {
            if (findUpEvent(false)) {
                ...
                return;
            } else if (mClickedAllday) {
                if (DEBUG_DPAD) {
                    Llog.d(mTempTime.format(
                        "No more all day events found, leaving %b%d all day selected"));
                }
                return;
            } else {
                if (DEBUG_DPAD) {
            ...
            if (DEBUG_DPAD) {
                Llog.d(mTempTime.format(
                    "Leaving %b%d all day selected"));
            }
            return;
        }
        if (mSelectionTime.hour == 0) {
            mClickedAllday = true;
        } else {
            mClickedHour = --mSelectionTime.hour;
            mTempTime.hour = mClickedHour;
        ...
        if (DEBUG_DPAD) {
            if (findUpEvent(true)) {
                ...
                if (mClickedAllday) {
                    Llog.d(mTempTime.format(
                        "No all day event found, leaving %b%d all day selected"));
                } else {
                    Llog.d(mTempTime.format(
                        "No event found, leaving %H:00 on %b%d selected"));
                }
            }
        } else {
            findUpEvent(true);
    ...
    // This does the work for nextDown().
    // In the case where there is a selected event, but it cannot find an event
    // below it in the selected day, it gets called a second time with allowSelected true.
    // Returns true if it found an event
    private boolean findDownEvent(boolean allowSelected) {
        if (DEBUG_DPAD) {
            Llog.d("***********************************************");
        }
        String selectedName = null;
        ...
        if (mClickedAllday) {
            if (DEBUG_DPAD) {
                if (mSelectedEvent == null) {
                    Llog.d(mTempTime.format(
                        "No previously selected event, looking for highest all day event on %b%d"));
                } else {
                    selectedName = ((mSelectedEvent.title == null)
                        ? "Untitled event" : mSelectedEvent.title).toString();
                    Llog.d(mTempTime.format("Looking for highest all day event on %b%d below "
                        + selectedName));
                }
            }
            int minColumn =
                ((mSelectedEvent != null) && mSelectedEvent.drawAsAllday())
                    ? mSelectedEvent.getColumn() : -1;
            ...
            for (int i = 0; i < numEvents; i++) {
                ...
                if (e.drawAsAllday()) {
                    ...
                    if (eColumn > maxColumn) {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName + " is below best so far, not wanted");
                        }
                        continue;
                    }
                    if (eColumn < minColumn) {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName + " is above selected event, not wanted");
                        }
                        continue;
                    }
                    if (DEBUG_DPAD) {
            ...
            if (DEBUG_DPAD) {
                if (mSelectedEvent == null) {
                    Llog.d(mTempTime.format("Looking for earliest event between %H:00") +
                        mCompareTime.format(" and %H:00 on %b%d"));
                } else {
                    selectedName = ((mSelectedEvent.title == null)
                        ? "Untitled event" : mSelectedEvent.title).toString();
                    Llog.d(mTempTime.format("Looking for earliest event between %H:00") +
                        mCompareTime.format(" and %H:00 on %b%d after " + selectedName));
                }
            }
            long endMillis = mCompareTime.toMillis(true);
            for (int i = 0; i < numEvents; i++) {
                ...
                if (mSelectedEvent != null) {
                        ...
                        && !allowSelected)
                    {
                        ...
                        if (DEBUG_DPAD) {
                            Llog.d(eventName +
                                ", not wanted because it is before " + selectedName);
                        }
                        continue;
                    }
                }
                if (mClickedEvent != null) {
                                           ...
                                           && (e.getColumn() > mClickedEvent.getColumn())))))
                    {
                        if (DEBUG_DPAD) {
                            Llog.d(eventName +
                                ", not wanted because it is after " + clickedName);
                        }
                        continue;
                    }
                }
    ...
    // This gets called on a DPAD_DOWN key down, which may be a repeat
    // It selects the nearest event below the currently selected event
    // if there is one, or the currently selected hour if no event is selected.
    // If there is no event within one hour upwards, it selects the next hour up.
    // If we move down from the all day event area, we go hour 00.
    // mClickedAllday, mClickedDay, mClickedHour, and mClickedEvent are set to reflect
    // what was found.
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (   (mClickedDay > mLastJulianDay)
            || (mClickedDay < mFirstJulianDay))
        {
            // moved out of current day or week,need to switch views
            DayView view = (DayView) mViewSwitcher.getNextView();
            view.mBaseDate.set(mBaseDate);
            if (mClickedDay < mFirstJulianDay) {
            ...
            view.setSelectedDay(mClickedDay);
        ...
        mController.sendAction(this, ControllerAction.GO_TO, mTempTime, mTempTime, -1, ViewType.CURRENT);
        return true;
    }
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if (KeyEvent.KEYCODE_DPAD_CENTER == keyCode) {
                ...
                if (mSelectionMode == SELECTION_PRESSED) {
                    ...
                    mSelectionMode = SELECTION_SELECTED;
                    invalidate();
                } else // Check the duration to determine if this was a short press
    ...
    private void sendAccessibilityEventAsNeeded(boolean speakEvents) {
        ...
        // FIXME all day click not handled correctly here
    ...
    /**
     * @param b         a StringBuilder to use
     * @param calEvent  the event for which to make an accessibility string
     */
    ...
    private void slideViews(boolean forward, float xOffSet, float width, float velocity) {
        mAnimationDistance = width - xOffSet;
        if (DEBUG) {
            Log.d(TAG, "slideViews(" + forward + ") O:" + xOffSet + " Dist:" + mAnimationDistance);
        }
    ...
    /* package */ void reloadEvents() {
        ...
        long millis = mBaseDate.toMillis(true /* ignore isDst */);
    ...
    private void computeAllDaySpace() {
        ...
        int maxAllDayEvents = mSelectionAllday ? 1 : 0;
    ...
    // all-day area.
    private void drawUpperLeftCorner(Canvas canvas, Paint p) {
    ...
    private void drawScrollLine(Canvas canvas, Paint p) {
    ...
    private void drawCurrentTimeLine(final int day, final int top, Canvas canvas,
            Paint p) {
        mRect.left = computeDayLeftPosition(day) - CURRENT_TIME_LINE_SIDE_BUFFER + 1;
        mRect.right = computeDayLeftPosition(day + 1) + CURRENT_TIME_LINE_SIDE_BUFFER + 1;

        mRect.top = top - CURRENT_TIME_LINE_TOP_OFFSET;
        mRect.bottom = mRect.top + mCurrentTimeLine.getIntrinsicHeight();

        mCurrentTimeLine.setBounds(mRect);
        mCurrentTimeLine.draw(canvas);
        if (mAnimateToday) {
            mCurrentTimeAnimateLine.setBounds(mRect);
    ...
    private void doDraw(Canvas canvas) {
        ...
        for (int day = 0; day < mNumDays; day++, cell++) {
            ...
            // but we need to be careful, an event can be in more than one day
    ...
    private void drawSelectedRect(Canvas canvas, Paint p) {
        // Draw a highlight on the selected hour
        if (mSelectionAllday) {
            mRect.top = DAY_HEADER_HEIGHT + 1;
            mRect.bottom = mRect.top + mAlldayHeight + ALLDAY_TOP_MARGIN - 2;
        } else {
            mRect.top = mSelectionTime.hour * (mCellHeight + HOUR_GAP);
            mRect.bottom = mRect.top + mCellHeight + HOUR_GAP;
        }
        ...
        /* uncomment when required for testing
        if (   (mRect.left != mPrevBox.left)
            || (mRect.top != mPrevBox.top)
            ...
            || (mRect.bottom != mPrevBox.bottom))
        {
            ...
            Llog.d("mSelectionAllday is " + ( mSelectionAllday ? "true" : "false"));
            Llog.d( "left " + mRect.left + ", top " + mRect.top
                + ", right " + mRect.right + ", bottom " + mRect.bottom);
            mPrevBox.left = mRect.left;
            mPrevBox.right = mRect.right;
            mPrevBox.top = mRect.top;
            mPrevBox.bottom = mRect.bottom;
        } //*/
    ...
    private void drawGridBackground(Canvas canvas, Paint p) {
     ...
     * @param p
     */
    private void drawBgColors(Canvas canvas, Paint p) {
        int todayIndex = mTodayJulianDay - mFirstJulianDay;
        // Draw the hours background color
        mRect.top = mDestRect.top;
        mRect.bottom = mDestRect.bottom;
        mRect.left = 0;
        mRect.right = mHoursWidth;
        p.setColor(mBgColor);
        p.setStyle(Style.FILL);
        ...
        if (mNumDays == 1 && todayIndex == 0) {
            ...
            if (lineY < mViewStartY + mViewHeight) {
                ...
                mRect.left = mHoursWidth;
                mRect.right = mViewWidth;
                mRect.top = lineY;
                mRect.bottom = mViewStartY + mViewHeight;
                p.setColor(mFutureBgColor);
                canvas.drawRect(mRect, p);
            }
            ...
            if (lineY < mViewStartY + mViewHeight) {
                lineY = Math.max(lineY, mViewStartY);
                mRect.left = computeDayLeftPosition(todayIndex) + 1;
                mRect.right = computeDayLeftPosition(todayIndex + 1);
                mRect.top = lineY;
                mRect.bottom = mViewStartY + mViewHeight;
                p.setColor(mFutureBgColor);
                canvas.drawRect(mRect, p);
            ...
            if (todayIndex + 1 < mNumDays) {
                mRect.left = computeDayLeftPosition(todayIndex + 1) + 1;
                mRect.right = computeDayLeftPosition(mNumDays);
                mRect.top = mDestRect.top;
                mRect.bottom = mDestRect.bottom;
                p.setColor(mFutureBgColor);
            ...
            // Future
            mRect.left = computeDayLeftPosition(0) + 1;
            mRect.right = computeDayLeftPosition(mNumDays);
            mRect.top = mDestRect.top;
            mRect.bottom = mDestRect.bottom;
            p.setColor(mFutureBgColor);
     ...
     */
    private StaticLayout getEventLayout(
        StaticLayout[] layouts, int i, Event event, Paint paint, Rect r)
    {
    ...
    private void drawAllDayEvents(Canvas canvas, Paint p) {
        ...
        int lastDay = mFirstJulianDay + mNumDays - 1;
        ...
        Arrays.fill(mSkippedAlldayEvents, 0);
        ...
        for (int i = 0; i < numEvents; i++) {
            ...
            // side-effect - sets mRect
            drawEventRect(event, canvas, p, eventTextPaint, (int) event.top,
                    (int) event.bottom);
            if ((mRect.bottom <= mRect.top) || (mRect.right <= mRect.left)) {
                continue;
            }

            if (mRect.bottom - mRect.top > EVENT_ALL_DAY_TEXT_TOP_MARGIN + EVENT_ALL_DAY_TEXT_BOTTOM_MARGIN) {
                mRect.top += EVENT_ALL_DAY_TEXT_TOP_MARGIN;
                mRect.bottom -= EVENT_ALL_DAY_TEXT_BOTTOM_MARGIN;
            }
            if (mRect.right - mRect.left > EVENT_ALL_DAY_TEXT_LEFT_MARGIN + EVENT_ALL_DAY_TEXT_RIGHT_MARGIN) {
                mRect.left += EVENT_ALL_DAY_TEXT_LEFT_MARGIN;
                mRect.right -= EVENT_ALL_DAY_TEXT_RIGHT_MARGIN;
            }
            StaticLayout layout = getEventLayout(mAllDayLayouts, i, event, eventTextPaint, mRect);
            drawEventText(layout, canvas, mRect.top, mRect.bottom, true);

            // Check if this all-day event intersects the selected day
            if (mSelectionAllday && mComputeSelectedEvents) {
                if (startDay <= mSelectionJulianDay && endDay >= mSelectionJulianDay) {
        ...
        if (mMoreAlldayEventsTextAlpha != 0) {
    ...
    protected void drawMoreAlldayEvents(
        Canvas canvas, int remainingEvents, int day, Paint p)
    {
        ...
        mRect.top = y;
        mRect.left = x;
        mRect.bottom = y + EVENT_SQUARE_WIDTH;
        mRect.right = x + EVENT_SQUARE_WIDTH;
    ...
    private void drawEvents(int date, int dayIndex, int top, Canvas canvas, Paint p) {
        ...
        // mSelectionRect is used in a loop in which mRect is clobbered
        // so it has to be different
        mSelectionRect.top = top + mSelectionTime.hour * (cellHeight + HOUR_GAP);
        mSelectionRect.bottom = mSelectionRect.top + cellHeight;
        mSelectionRect.left = left;
        mSelectionRect.right = mSelectionRect.left + cellWidth;
        ...
        for (int i = 0; i < numEvents; i++) {
            ...
            if (date == mSelectionDay && !mSelectionAllday && mComputeSelectedEvents
                    && geometry.eventIntersectsSelection(event, mSelectionRect)) {
                mSelectedEvents.add(event);
            }

            // side-effect - sets mRect
            drawEventRect(event, canvas, p, eventTextPaint, mViewStartY, viewEndY);
            if (mRect.bottom <= mRect.top || mRect.right <= mRect.left) {
                continue;
            }

            if (mRect.bottom - mRect.top > EVENT_TEXT_TOP_MARGIN + EVENT_TEXT_BOTTOM_MARGIN) {
                mRect.top += EVENT_TEXT_TOP_MARGIN;
                mRect.bottom -= EVENT_TEXT_BOTTOM_MARGIN;
            }
            if (mRect.right - mRect.left > EVENT_TEXT_LEFT_MARGIN + EVENT_TEXT_RIGHT_MARGIN) {
                mRect.left += EVENT_TEXT_LEFT_MARGIN;
                mRect.right -= EVENT_TEXT_RIGHT_MARGIN;
            }

            // Don't draw text if it is not visible
            if (mRect.top > viewEndY || mRect.bottom < mViewStartY) {
                continue;
            }
            StaticLayout layout = getEventLayout(mLayouts, i, event, eventTextPaint, mRect);
            // TODO: not sure why we are 4 pixels off
            drawEventText(layout, canvas, mViewStartY + 4, mViewStartY + mViewHeight
                    - DAY_HEADER_HEIGHT - mAlldayHeight, false);
        }
        eventTextPaint.setAlpha(alpha);
    }

    // This leaves mRect set up for drawEventText
    private void drawEventRect(Event event, Canvas canvas, Paint p, Paint eventTextPaint,
            int visibleTop, int visibleBot) {
        // Draw the Event Rect

        // If we clicked on an event, we highlight it
        // If there is no clicked event and there is a selected event, we highlight that
        int color;
        if (event.equals(mClickedEvent)) {
            color = mClickedColor;
        } else if ((event.equals(mSelectedEvent)) && (mClickedEvent == null)) {
            color =  mPressedColor;
        } else {
            color = event.color;
        }
        ...
        mRect.top = Math.max((int) event.top + EVENT_RECT_TOP_MARGIN + floorHalfStroke, visibleTop);
        mRect.bottom = Math.min((int) event.bottom - EVENT_RECT_BOTTOM_MARGIN - ceilHalfStroke,
                visibleBot);
        mRect.left = (int) event.left + EVENT_RECT_LEFT_MARGIN + floorHalfStroke;
        ...
        mRect.bottom = (int) event.bottom - EVENT_RECT_BOTTOM_MARGIN;
        mRect.left = (int) event.left + EVENT_RECT_LEFT_MARGIN;
        mRect.right = (int) event.right - EVENT_RECT_RIGHT_MARGIN;
    }
    ...
    private void drawEventText(StaticLayout eventLayout, Canvas canvas,
                               int top, int bottom, boolean center)
    {
        ...
        if (totalLineHeight == 0 || mRect.top > bottom || mRect.top + totalLineHeight < top)
        {
            return;
        }
        ...
        canvas.save();
        int padding = center ? (mRect.bottom - mRect.top - totalLineHeight) / 2 : 0;
        canvas.translate(mRect.left, mRect.top + padding);
        mRect.left = 0;
        mRect.right = width;
        mRect.top = 0;
        mRect.bottom = totalLineHeight;
    ...
    // This is called from the CalendarGestureListener as soon as the user does a touch,
    // before we know yet whether it going to be a click, a long press, a scroll,
    // a fling, or a scale. If the touch was on an event, we arrange to highlight it.
    // If it eventually turns out to be a long press or a scroll or a fling or a scale,
    // the handlers for those will remove the highlighting.
    private void doDown(MotionEvent ev) {
        mTouchMode = TOUCH_MODE_DOWN;
        mViewStartX = 0;
        mOnFlingCalled = false;
        mHandler.removeCallbacks(mContinueScroll);
        // We only highlight touched events here.
        // The touched hour will be highlighted later if we decide it was
        // a click or a long press and not a scroll or a fling or a scale
        if (setSelectionFromPosition((int) ev.getX(), (int) ev.getY())) {
            if (mClickedEvent != null) {
                // redraw will highlight clicked event
                invalidate();
            }
        }
    }
    ...
    private void doSingleTapUp(MotionEvent ev) {
        ...
        if (y < DAY_HEADER_HEIGHT) {
            ...
            if (x >= mHoursWidth) {
                ...
                if (mClickedDay < mNumDays) {
                    ...
                    mController.sendAction(this, ControllerAction.GO_TO,
        ...
        // mClickedHour, mClickedDay, mClickedEvent, and mClickedAllday

        mTouchMode = TOUCH_MODE_INITIAL_STATE;
        if (mClickedEvent != null) {
            // Tap on event, view it

            // Select the event and also the time in case not already selected
            setSelectionTime(mClickedDay, mClickedHour);
            mSelectionAllday = mClickedAllday;
            setSelectedEvent(mClickedEvent);
            ...
            // because the selected event may get deleted
            clearCachedEvents();
            mController.sendEventAction(this, ControllerAction.EDIT_EVENT,
                mSelectedEvent.id,
                   ...
                   && (mClickedAllday == mSelectionAllday))
        {
            ...
            mController.sendEventActionWithExtra(this,
    ...
    private void doLongPress(MotionEvent ev) {
        if (mScrolling || (mStartingSpanY != 0)) {
            return;
        }
    ...
    private void doFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY)
    {
        ...
        if ((mTouchMode & TOUCH_MODE_HSCROLL) != 0) {
            ...
            slideViews(deltaX < 0, mViewStartX, mViewWidth, velocityX);
            mViewStartX = 0;
            return;
        }
    ...
    private void initNextView(int deltaX) {
    ...
    // FIXME This seems to be very poor design:
    // Either we should do everything here or we should do everything
    // in the callbacks from the gesture listeners. Doing some work here and
    // some elsewhere is bound to lead to maintainability issues.
    @Override
            ...
            case MotionEvent.ACTION_UP:
                ...
                if ((mTouchMode & TOUCH_MODE_HSCROLL) != 0) {
                    ...
                    if (Math.abs(mViewStartX) > mHorizontalSnapBackThreshold) {
                        ...
                        slideViews(mViewStartX > 0, mViewStartX, mViewWidth, 0);
                        mViewStartX = 0;
                        return true;
                    } else {
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuInfo)
    {
        ...
        final long startMillis = mSelectionTime.toMillis(true);
    ...
    /**
     * Sets mClickedAllday. mClickedDay, mClickedHour, and mClickedEvent
    ...
    private boolean setSelectionFromPosition(int x, int y)
    {
        ...
        int numEvents = events.size();
        int top = mFirstHourOffset + mFirstCell - mFirstHour * (mCellHeight + HOUR_GAP);
        if (x < mHoursWidth) {
            if (y < mFirstCell) { return false; } // nothing to select here
            x = mHoursWidth;
        }
        ...
        if (y < mFirstCell) {
            ...
            if (mSelectedEvents.size() > 0) {
                ...
                if (mClickedHour < startHour && mSelectionJulianDay == mClickedDay) {
                    mClickedHour = startHour;
                } else if (mClickedHour > endHour && mSelectionJulianDay == mClickedDay) {
                    mClickedHour = endHour;
                }
            }
        }

        mSelectionDayForAccessibility = mClickedDay;
        mSelectionHourForAccessibility = mClickedHour;
        mSelectedEventForAccessibility = mClickedEvent;
    ...
    private class GotoBroadcaster implements Animation.AnimationListener {
        ...
        public void onAnimationEnd(Animation animation) {
            ...
            if (mCounter == sCounter) {
                mController.sendAction(this, ControllerAction.GO_TO, mStart, mEnd, null, -1,
    ...
    private class ContextMenuHandler implements MenuItem.OnMenuItemClickListener {
        ...
        public boolean onMenuItemClick(MenuItem item) {
            switch (item.getItemId()) {
                ...
                case MENU_DAY: {
                    mController.sendAction(this, ControllerAction.GO_TO, getSelectedTime(), null, -1,
                            ViewType.DAY);
                    break;
                }
                case MENU_AGENDA: {
                ...
                case MENU_EVENT_CREATE: {
                    ...
                    long endMillis = startMillis + DateUtils.HOUR_IN_MILLIS;
                    mController.sendEventAction(this, ControllerAction.CREATE_EVENT, -1,
                ...
                case MENU_EVENT_DELETE: {
                    if (mSelectedEvent != null) {
                        ...
                        // because the selected event may get deleted
                        clearCachedEvents();
                        mController.sendEventAction(this, ControllerAction.DELETE_EVENT, id, begin,
                                end, 0, 0, -1);
                    }
                    break;
                }
    ...
    private void dumpOneEvent(StringBuilder sb, Event e) {
        ...
        t.set(e.startMillis);
        sb.append(t.format(" start %b%d %H:%M " + e.startMillis));
        t.set(e.endMillis);
        sb.append(t.format(", end %b%d %H:%M " + e.endMillis));
